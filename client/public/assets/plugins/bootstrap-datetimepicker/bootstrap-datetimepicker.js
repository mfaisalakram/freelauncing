...
!(function ($) {
  ...
  var Datetimepicker = function (element, options) {
    ...
    this.language =
      this.language in dates ? this.language : this.language.split('-')[0]; // fr-CA fallback to fr
    ...
    this.linkFieldd =
      options.linkFieldd || this.element.data('link-fieldd') || false;
    ...
    this.linkFormatt = DPGlobal.parseFormat(
      options.linkFormatt ||
        this.element.data('link-formatt') ||
        DPGlobal.getDefaultFormat(this.formatType, 'link'),
      this.formatType
    );
    this.minuteStep =
  ...
  Datetimepicker.prototype = {
    ...
    setValue: function () {
      ...
      if (this.linkFieldd) {
        $('#' + this.linkFieldd).val(this.getFormattedDate(this.linkFormatt));
      }
    },
    ...
    fill: function () {
      ...
      var d = new Date(this.viewDate),
        ...
        startMonth =
          this.startDate !== -Infinity
            ? this.startDate.getUTCMonth() + 1
            : -Infinity,
        endYear =
          this.endDate !== Infinity ? this.endDate.getUTCFullYear() : Infinity,
        endMonth =
          this.endDate !== Infinity ? this.endDate.getUTCMonth() + 1 : Infinity,
      ...
      for (var i = 0; i < 24; i++) {
        ...
        if (this.showMeridian && dates[this.language].meridiem.length === 2) {
          ...
          if (meridian != meridianOld) {
            ...
            html.push(
              '<fieldset className="hour"><legend>' +
          ...
          txt = i + ':00';
          html.push('<span className="hour' + clsName + '">' + txt + '</span>');
        }
      }
      ...
      for (var i = 0; i < 60; i += this.minuteStep) {
        ...
        if (this.showMeridian && dates[this.language].meridiem.length === 2) {
          ...
          if (meridian != meridianOld) {
            ...
            html.push(
              '<fieldset className="minute"><legend>' +
                meridian.toUpperCase() +
                '</legend>'
            );
          }
          ...
          //html.push('<span className="minute'+clsName+' minute_'+(hours<12?'am':'pm')+'">'+txt+'</span>');
          html.push(
  ...
  var DPGlobal = {
    ...
    headTemplate:
      '<thead>' +
      '<tr>' +
      '<th className="prev"><i className="{iconType} {leftArrow}"/></th>' +
      '<th colspan="5" className="switch"></th>' +
      '<th className="next"><i className="{iconType} {rightArrow}"/></th>' +
      '</tr>' +
      '</thead>',
    headTemplateV3:
      '<thead>' +
      '<tr>' +
      '<th className="prev"><span className="{iconType} {leftArrow}"></span> </th>' +
      '<th colspan="5" className="switch"></th>' +
      '<th className="next"><span className="{iconType} {rightArrow}"></span> </th>' +
      '</tr>' +
      '</thead>',
    contTemplate: '<tbody><tr><td colspan="7"></td></tr></tbody>',
    footTemplate:
      '<tfoot><tr><th colspan="7" className="today"></th></tr></tfoot>',
  };
  DPGlobal.template =
    '<div className="datetimepicker">' +
    '<div className="datetimepicker-minutes">' +
    ...
    '</table>' +
    '</div>' +
    '<div className="datetimepicker-hours">' +
    '<table className=" table-condensed">' +
    DPGlobal.headTemplate +
    DPGlobal.contTemplate +
    DPGlobal.footTemplate +
    '</table>' +
    '</div>' +
    '<div className="datetimepicker-days">' +
    '<table className=" table-condensed">' +
    DPGlobal.headTemplate +
    '<tbody></tbody>' +
    DPGlobal.footTemplate +
    '</table>' +
    '</div>' +
    '<div className="datetimepicker-months">' +
    '<table className="table-condensed">' +
    DPGlobal.headTemplate +
    DPGlobal.contTemplate +
    DPGlobal.footTemplate +
    '</table>' +
    '</div>' +
    '<div className="datetimepicker-years">' +
    '<table className="table-condensed">' +
    DPGlobal.headTemplate +
    DPGlobal.contTemplate +
    DPGlobal.footTemplate +
