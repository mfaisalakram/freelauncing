    ...
    create: function () {
      $('.progressUploadAnimate .ui-progressbar-value').animate(
        { width: '100%' },
        {
          ...
          easing: 'linear',
          step: function (now) {
            $('.progressUploadAnimateValue').html(
              parseInt(now * 40.96) + ' Gb'
            );
          },
          complete: function () {
            $('.progressUploadAnimate + .field_notice').html(
              "<span className='must'>Upload Finished</span>"
            );
          },
        }
      );
