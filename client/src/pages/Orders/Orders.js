...
const Orders = (props) => {
  ...
  useEffect(() => {
    props.getOrders('active', props.history);
    getCount();
  }, []);
  ...
  const getCount = () => {
    // axios
    //   .get("http://localhost:5000/api/orders/count/by-stats/services/count")
    //   .then(response => {
  ...
  const onServiceTagClick = (e, tag) => {
    props.getOrders(tag, props.history);
    setActive(tag);
  };
  ...
  if (props.loadingOrders === false) {
    if (props.loading === false && props.isAuthenticated) {
      if (props.user.current_type === 'buyer')
          ...
          <div className="col-md-12">
            <div>
              <div className="servicesTypeButtons">
                ...
                <span
                  onClick={(e) => {
                    onServiceTagClick(e, 'late');
                  }}
                  style={active === 'late' ? { fontWeight: '600' } : {}}
                >
                  late
                  {/* {counts.late && (
                    <span className="service-count">{counts.late}</span>
                  )} */}
                </span>
                <span
                  onClick={(e) => {
