  ...
  var i,
    ...
    l = function (t, e) {
      (this.rawQuery = t.val() || ''),
        ...
        (this.node = t),
        (this.namespace =
          '.' + this.helper.slugify.call(this, this.selector) + '.typeahead'),
        (this.isContentEditable =
          void 0 !== this.node.attr('contenteditable') &&
          'false' !== this.node.attr('contenteditable')),
        (this.container = null),
  ...
  (l.prototype = {
    _validateCacheMethod: function (t) {
      ...
      if (!0 === t) t = 'localStorage';
        ...
        'string' === typeof t &&
        !~['localStorage', 'sessionStorage'].indexOf(t)
      )
        return !1;
      e = void 0 !== window[t];
      try {
        window[t].setItem('typeahead', 'typeahead'),
          window[t].removeItem('typeahead');
      } catch (t) {
        e = !1;
      }
      return (e && t) || !1;
    ...
    extendOptions: function () {
      if (
        ...
        this.options.multiselect &&
          ((this.items = []),
          (this.comparedItems = []),
          'string' === typeof this.options.multiselect.matchOn &&
            (this.options.multiselect.matchOn = [
              this.options.multiselect.matchOn,
            ])),
        this.options.group &&
          (Array.isArray(this.options.group) ||
            ('string' === typeof this.options.group
              ? (this.options.group = { key: this.options.group })
              : 'boolean' === typeof this.options.group &&
        ...
        for (var t = 0, e = this.options.dropdownFilter.length; t < e; ++t)
          this.dropdownFilter[
            this.options.dropdownFilter[t].value ? 'static' : 'dynamic'
      ...
      this.options.dynamicFilter &&
        ...
        this.options.accent &&
          ('object' === typeof this.options.accent
            ? this.options.accent.from &&
              this.options.accent.to &&
              (this.options.accent.from.length, this.options.accent.to.length)
            : (this.options.accent = o)),
        this.options.groupTemplate &&
          (this.groupTemplate = this.options.groupTemplate),
        this.options.resultContainer &&
          ('string' === typeof this.options.resultContainer &&
            (this.options.resultContainer = j(this.options.resultContainer)),
          this.options.resultContainer instanceof j &&
    ...
    unifySourceFormat: function () {
      ...
      this.options.source.ajax &&
        (this.options.source = { group: { ajax: this.options.source.ajax } }),
      ...
      this.options.source))
        if (this.options.source.hasOwnProperty(t)) {
          if (
            ('string' === typeof (e = this.options.source[t]) &&
              (e = { ajax: { url: e } }),
            (i = e.url || e.ajax),
            Array.isArray(i)
              ? ((e.ajax = 'string' === typeof i[0] ? { url: i[0] } : i[0]),
                (e.ajax.path = e.ajax.path || i[1] || null))
              : 'object' === typeof e.url
              ? (e.ajax = e.url)
          ...
          e.display && !Array.isArray(e.display) && (e.display = [e.display]),
            ...
            (e.dynamic =
              'boolean' === typeof e.dynamic || this.options.dynamic),
            ...
            (e.cache =
              void 0 !== e.cache
                ? this._validateCacheMethod(e.cache)
                : this.options.cache),
            e.compression &&
              (('object' === typeof LZString && e.cache) ||
                (e.compression = !1));
        }
      return (
        ...
        switch (t.type) {
          ...
          case 'keydown':
            ...
            i.items.length
              ...
              : t.keyCode &&
                ~[9, 13, 27, 38, 39, 40].indexOf(t.keyCode) &&
                ((s = !0), i.navigate(t));
            break;
          case 'keyup':
            n &&
          ...
          case 'input':
            (i.deferred = j.Deferred()),
              ...
              '' === i.rawQuery &&
                '' === i.query &&
                ((t.originalEvent = e || {}),
                i.helper.executeCallback.call(i, i.options.callback.onCancel, [
                  i.node,
                  i.item,
                  t,
                ]),
              ...
              i.options.hint &&
                ...
                0 !== i.hint.container.val().indexOf(i.rawQuery) &&
                (i.hint.container.val(''),
    ...
    filterGenerateSource: function () {
      if (
        ...
        for (var t in this.options.source)
          if (
            this.options.source.hasOwnProperty(t) &&
            this.query.length >= this.options.source[t].minLength &&
            this.query.length <= this.options.source[t].maxLength
          ) {
            if (
              this.filters.dropdown &&
              'group' === this.filters.dropdown.key &&
              this.filters.dropdown.value !== t
            )
              continue;
            if (
              (this.searchGroups.push(t),
              !this.options.source[t].dynamic && this.source[t])
            )
              continue;
    ...
    generateSource: function (t) {
      if ((this.filterGenerateSource(), Array.isArray(t) && t.length))
        this.generateGroups = t;
      else if (!this.generateGroups.length)
        return void this.node.trigger('search' + this.namespace);
      if (
        ((this.requestGroups = []),
        (this.generatedGroupCount = 0),
      ...
      for (
        var i,
          ...
          l,
          h = this,
          c = ((e = 0), this.generateGroups.length);
        e < c;
        ++e
      ) {
        if (
          ((i = this.generateGroups[e]),
          (n = (o = this.options.source[i]).cache),
          (r = o.compression),
          n && (a = window[n].getItem('TYPEAHEAD_' + this.selector + ':' + i)))
        ) {
          r && (a = LZString.decompressFromUTF16(a)), (l = !1);
          try {
            (a = JSON.parse(a + '')).data && a.ttl > new Date().getTime()
              ? (this.populateSource(a.data, i), (l = !0))
              : window[n].removeItem('TYPEAHEAD_' + this.selector + ':' + i);
          } catch (t) {}
          if (l) continue;
        }
        !o.data || o.ajax
          ? o.ajax &&
            (this.requests[i] ||
              (this.requests[i] = this.generateRequestObject(i)),
            this.requestGroups.push(i))
    ...
    generateRequestObject: function (s) {
      ...
      if (
        'function' != typeof n.ajax &&
        (n.ajax instanceof Object && (t = this.extendXhrObject(t, n.ajax)),
        1 < Object.keys(this.options.source).length)
      )
        for (var e in this.requests)
          this.requests.hasOwnProperty(e) &&
            (this.requests[e].isDuplicated ||
              (t.request.url &&
                t.request.url === this.requests[e].request.url &&
                (this.requests[e].validForGroup.push(s),
                (t.isDuplicated = !0),
                delete t.validForGroup)));
      return t;
    },
    extendXhrObject: function (t, e) {
      return (
        'object' === typeof e.callback &&
          ((t.callback = e.callback), delete e.callback),
        'function' === typeof e.beforeSend &&
          ((t.callback.beforeSend = e.beforeSend), delete e.beforeSend),
        (t.request = j.extend(!0, t.request, e)),
        'jsonp' !== t.request.dataType.toLowerCase() ||
          t.request.jsonpCallback ||
          (t.request.jsonpCallback = 'callback_' + t.extra.group),
        t
      );
    },
            ...
            this.requests[t].isDuplicated ||
              (function (t, r) {
                if ('function' === typeof h.options.source[t].ajax) {
                  ...
                  if (
                    'object' !=
                      typeof (r = h.extendXhrObject(
                        h.generateRequestObject(t),
                        'object' === typeof e ? e : {}
                      )).request ||
                    !r.request.url
                  )
                    return h.populateSource([], t);
                  h.requests[t] = r;
                }
                var a,
                  i = !1,
                  l = {};
                if (
                  (~r.request.url.indexOf('{{query}}') &&
                    (i || ((r = j.extend(!0, {}, r)), (i = !0)),
                    (r.request.url = r.request.url.replace(
                      '{{query}}',
                      encodeURIComponent(h.query)
                    ))),
                  r.request.data)
                )
                  for (var s in r.request.data)
                    if (
                      r.request.data.hasOwnProperty(s) &&
                      ~String(r.request.data[s]).indexOf('{{query}}')
                    ) {
                      i || ((r = j.extend(!0, {}, r)), (i = !0)),
                        (r.request.data[s] = r.request.data[s].replace(
                          '{{query}}',
                          h.query
                        ));
                      break;
                ...
                j.ajax(r.request)
                  .done(function (t, e, i) {
                    for (var s, o = 0, n = r.validForGroup.length; o < n; o++)
                      (s = r.validForGroup[o]),
                        'function' ==
                          typeof (a = h.requests[s]).callback.done &&
                          (l[s] = a.callback.done.call(h, t, e, i));
                  })
                  .fail(function (t, e, i) {
                    for (var s = 0, o = r.validForGroup.length; s < o; s++)
                      (a = h.requests[r.validForGroup[s]]).callback
                        .fail instanceof Function &&
                        a.callback.fail.call(h, t, e, i);
                  })
                  .always(function (t, e, i) {
                    for (var s, o = 0, n = r.validForGroup.length; o < n; o++) {
                      if (
                        ((s = r.validForGroup[o]),
                        (a = h.requests[s]).callback.always instanceof
                          Function && a.callback.always.call(h, t, e, i),
                        'abort' === e)
                      )
                        return;
                      h.populateSource(
                        (null !== t && 'function' === typeof t.promise && []) ||
                          l[s] ||
                          t,
                        a.extra.group,
                        a.extra.path || a.request.path
                      ),
                        0 === (c -= 1) &&
                          h.helper.executeCallback.call(
                            h,
                            h.options.callback.onReceiveRequest,
                            [h.node, h.query]
                          );
                    }
                  })
                  .then(function (t, e) {
                    for (var i = 0, s = r.validForGroup.length; i < s; i++)
                      (a = h.requests[r.validForGroup[i]]).callback
                        .then instanceof Function &&
                        a.callback.then.call(h, t, e);
                  });
              })(t, this.requests[t]);
    },
    populateSource: function (i, t, e) {
      ...
      e &&
        'string' === typeof e &&
        (i = this.helper.namespace.call(this, e, i)),
        Array.isArray(i) || (i = []),
        n &&
      ...
      for (
        var r,
          a = o.display
            ? 'compiled' === o.display[0]
              ? o.display[1]
              : o.display[0]
            : 'compiled' === this.options.display[0]
        ...
        null !== i[l] &&
          'boolean' != typeof i[l] &&
          ('string' === typeof i[l] && (((r = {})[a] = i[l]), (i[l] = r)),
          (i[l].group = t));
        ...
        for (l = 0, h = i.length; l < h; l++)
          for (var d = 0, f = this.dropdownFilter.dynamic.length; d < f; d++)
            (c = this.dropdownFilter.dynamic[d].key),
              (p = i[l][c]) &&
                (this.dropdownFilter.dynamic[d].value ||
                  (this.dropdownFilter.dynamic[d].value = []),
                u[c] || (u[c] = []),
                ~u[c].indexOf(p.toLowerCase()) ||
                  (u[c].push(p.toLowerCase()),
                  this.dropdownFilter.dynamic[d].value.push(p)));
      }
      if (this.options.correlativeTemplate) {
        ...
        if (('function' === typeof m && (m = m.call(this, '', {})), m)) {
          if (Array.isArray(this.options.correlativeTemplate))
            for (l = 0, h = this.options.correlativeTemplate.length; l < h; l++)
          ...
          for (l = 0, h = i.length; l < h; l++)
            i[l].compiled = j('<textarea />')
              ...
              .text();
          o.display
            ? ~o.display.indexOf('compiled') || o.display.unshift('compiled')
      ...
      this.options.callback.onPopulateSource &&
        ...
        (this.tmpSource[t] = (Array.isArray(i) && i) || []);
      ...
      if (y && !window[y].getItem('TYPEAHEAD_' + this.selector + ':' + t)) {
        ...
        v && (k = LZString.compressToUTF16(k)),
          window[y].setItem('TYPEAHEAD_' + this.selector + ':' + t, k);
      }
    ...
    incrementGeneratedGroup: function () {
      if (
        ...
        this.xhr = {};
        for (var t = 0, e = this.generateGroups.length; t < e; t++)
          this.source[this.generateGroups[t]] =
            this.tmpSource[this.generateGroups[t]];
    ...
    navigate: function (t) {
      if (
        (this.helper.executeCallback.call(
          this,
          this.options.callback.onNavigateBefore,
          [this.node, this.query, t]
        ),
        27 === t.keyCode)
      )
        return (
          t.preventDefault(),
          void (this.query.length
            ? (this.resetInput(),
              this.node.trigger('input' + this.namespace, [t]))
            : (this.node.blur(), this.hideLayout()))
        );
      if (this.result.length) {
        var e,
          i = this.resultContainer
        ...
        if (
          ...
          this.helper.executeCallback.call(
            ...
            [
              this.node,
              (null !== o && i.eq(o)) || void 0,
              (null !== n && this.result[n]) || void 0,
              t,
            ]
          ),
          13 === t.keyCode)
          ...
          return (
            t.preventDefault(),
            void (0 < s.length
              ? 'javascript:;' === s.find('a:first')[0].href
                ? s.find('a:first').trigger('click', t)
                : s.find('a:first')[0].click()
        ...
        if (39 !== t.keyCode) {
          9 === t.keyCode
            ...
            ? (t.preventDefault(),
              0 < s.length
                ? 0 <= o - 1 && ((r = o - 1), this.addActiveItem(i.eq(r)))
                : i.length &&
                  ((r = i.length - 1), this.addActiveItem(i.last())))
            : 40 === t.keyCode &&
              (t.preventDefault(),
              0 < s.length
                ? o + 1 < i.length && ((r = o + 1), this.addActiveItem(i.eq(r)))
                : i.length && ((r = 0), this.addActiveItem(i.first()))),
            ...
            this.helper.executeCallback.call(
              ...
              [
                this.node,
                (null !== r && i.eq(r)) || void 0,
                (null !== e && this.result[e]) || void 0,
                t,
              ]
            ),
            t.preventInputChange &&
              ~[38, 40].indexOf(t.keyCode) &&
              this.buildHintLayout(
                null !== e && e < this.result.length ? [this.result[e]] : null
              ),
            this.options.hint &&
              this.hint.container &&
              this.hint.container.css(
                ...
                t.preventInputChange
                  ? this.hint.css.color
                  : (null === e && this.hint.css.color) ||
                      this.hint.container.css('background-color') ||
                      'fff'
              );
          var a =
            null === e || t.preventInputChange
              ? this.rawQuery
              : this.getTemplateValue.call(this, this.result[e]);
          this.node.val(a),
            ...
            this.helper.executeCallback.call(
              ...
              [
                this.node,
                i,
                (null !== r && i.eq(r).find('a:first')) || void 0,
                (null !== e && this.result[e]) || void 0,
                this.query,
                t,
              ]
    ...
    searchResultData: function () {
      ...
      for (var v = 0, b = this.searchGroups.length; v < b; ++v)
        if (
          ((F = this.searchGroups[v]),
          !this.filters.dropdown ||
            'group' !== this.filters.dropdown.key ||
            this.filters.dropdown.value === F)
        ) {
          (o =
            void 0 !== this.options.source[F].filter
              ? this.options.source[F].filter
              : this.options.filter),
            (r =
              ('function' === typeof this.options.source[F].matcher &&
                this.options.source[F].matcher) ||
              y);
          for (
            var k = 0, w = this.source[F].length;
            k < w &&
            (!(this.resultItemCount >= f) || this.options.callback.onResult);
            k++
          )
            if (
              (!g ||
                this.dynamicFilter.validate.apply(this, [this.source[F][k]])) &&
              null !== (t = this.source[F][k]) &&
              'boolean' != typeof t &&
              (!this.options.multiselect || this.isMultiselectUniqueData(t)) &&
              (!this.filters.dropdown ||
                (t[this.filters.dropdown.key] || '').toLowerCase() ===
                  (this.filters.dropdown.value || '').toLowerCase())
            ) {
              if (
                ((u = 'group' === p ? F : t[p] ? t[p] : t.group) &&
                  !this.tmpResult[u] &&
                  ((this.tmpResult[u] = []), (this.resultCountPerGroup[u] = 0)),
                m &&
                  'group' === p &&
                  this.tmpResult[u].length >= m &&
                  !this.options.callback.onResult)
              )
                break;
              for (
                var x = 0,
                  C = (S =
                    this.options.source[F].display || this.options.display)
                    .length;
                x < C;
                ++x
              ) {
                if (!1 !== o) {
                  if (
                    void 0 ===
                      (s = /\./.test(S[x])
                        ? this.helper.namespace.call(this, S[x], t)
                        : t[S[x]]) ||
                    '' === s
                  )
                    continue;
                ...
                if (
                  (this.resultCount++,
                  this.resultCountPerGroup[u]++,
                  this.resultItemCount < f)
                ) {
                  if (m && this.tmpResult[u].length >= m) break;
                  this.tmpResult[u].push(j.extend(!0, { matchedKey: S[x] }, t)),
        ...
        for (var F in this.tmpResult)
          if (this.tmpResult.hasOwnProperty(F)) {
            for (v = 0, b = this.tmpResult[F].length; v < b; v++)
              (O =
                this.options.source[this.tmpResult[F][v].group].display ||
                this.options.display),
                ~S.indexOf(O[0]) || S.push(O[0]);
        ...
        v = 0,
          b = (I =
            'function' === typeof this.options.groupOrder
              ? this.options.groupOrder.apply(this, [
                  this.node,
                  this.query,
                  this.tmpResult,
                  this.resultCount,
    ...
    buildLayout: function () {
      this.buildHtmlLayout(),
        ...
        this.tmpResultHtml instanceof j
          ? this.resultContainer.html(this.tmpResultHtml)
          : this.resultHtml instanceof j &&
      ...
      if (!1 !== this.options.resultContainer) {
        ...
        for (
          var e,
            ...
            g,
            y,
            v = (this.groupTemplate && this.result.length && c.groups) || [],
            b = 0,
            k = this.result.length;
          b < k;
          ++b
        )
          (e = (n = this.result[b]).group),
            (s =
              (!this.options.multiselect &&
                this.options.source[n.group].href) ||
              this.options.href),
            (u = []),
            ...
            this.options.group &&
              ...
              this.options.group.template &&
                ('function' === typeof this.options.group.template
                  ...
                  : 'string' === typeof this.options.group.template &&
                    (i = this.options.group.template.replace(
                      /\{\{([\w\-\.]+)}}/gi,
                      function (t, e) {
              ...
              t.find('[data-search-group="' + e + '"]')[0] ||
                (this.groupTemplate
                  ? t.find('[data-group-template="' + e + '"] ul')
                  : t
                ).append(
                  j('<li/>', {
            ...
            this.groupTemplate &&
              v.length &&
              ~(m = v.indexOf(e || n.group)) &&
              v.splice(m, 1),
            (r = j('<li/>', {
              class:
                c.options.selector.item +
                ' ' +
                c.options.selector.group +
                '-' +
                this.helper.slugify.call(this, e),
              disabled: !!n.disabled,
              'data-group': e,
              'data-index': b,
              html: j('<a/>', {
                href:
                  s && !n.disabled
                    ? ((g = s),
                      (y = n),
                      (y.href = c.generateHref.call(c, g, y)))
                    : 'javascript:;',
                html: function () {
                  if (
                    (a =
                      (n.group && c.options.source[n.group].template) ||
                      c.options.template)
                  )
                    'function' === typeof a && (a = a.call(c, c.query, n)),
                      (l = a.replace(
                        /\{\{([^\|}]+)(?:\|([^}]+))*}}/gi,
                        function (t, e, i) {
                          var s = c.helper.cleanStringFromScript(
                            String(c.helper.namespace.call(c, e, n, 'get', ''))
                          );
                          return (
                            ~(i = (i && i.split('|')) || []).indexOf(
                              'slugify'
                            ) && (s = c.helper.slugify.call(c, s)),
                            ~i.indexOf('raw') ||
                              (!0 === c.options.highlight &&
                                o &&
                                ~d.indexOf(e) &&
                                (s = c.helper.highlight.call(
                                  c,
                                  s,
                                  o.split(' '),
                                  c.options.accent
                    ...
                    for (var t = 0, e = d.length; t < e; t++)
                      void 0 !==
                        (f = /\./.test(d[t])
                          ? c.helper.namespace.call(c, d[t], n, 'get', '')
                          : n[d[t]]) &&
                        '' !== f &&
                        u.push(f);
                    l =
                      '<span className="' +
                      c.options.selector.display +
                      '">' +
                      c.helper.cleanStringFromScript(String(u.join(' '))) +
                      '</span>';
                  }
                  ((!0 === c.options.highlight && o && !a) ||
                    'any' === c.options.highlight) &&
                    (l = c.helper.highlight.call(
                      c,
                      l,
                      o.split(' '),
                      c.options.accent
                  ...
                  : (e && 'object' === typeof e && (t.originalEvent = e),
                    c.options.mustSelectItem && c.helper.isEmpty(i)
                      ...
                      : (c.options.multiselect || (c.item = i),
                        !1 !==
                          c.helper.executeCallback.call(
                            c,
                            c.options.callback.onClickBefore,
                            [c.node, j(this), i, t]
                          ) &&
                          ((t.originalEvent &&
                            ...
                            (c.options.multiselect
                              ? ((c.query = c.rawQuery = ''),
                                c.addMultiselectItemLayout(i))
                              : ((c.focusOnly = !0),
                                (c.query = c.rawQuery =
                                  c.getTemplateValue.call(c, i)),
                                c.isContentEditable &&
                                  (c.node.text(c.query),
                                  c.helper.setCaretAtEnd(c.node[0]))),
                            c.hideLayout(),
                            c.node.val(c.query).focus(),
                            c.options.cancelButton &&
                              c.toggleCancelButtonVisibility(),
                            c.helper.executeCallback.call(
                              c,
                              c.options.callback.onClickAfter,
                              [c.node, j(this), i, t]
                            )))));
              }),
                e.on('mouseenter', function (t) {
                  i.disabled || (c.clearActiveItem(), c.addActiveItem(j(this))),
                    c.helper.executeCallback.call(
                      c,
                      c.options.callback.onEnter,
                      [c.node, j(this), i, t]
                    );
                }),
                e.on('mouseleave', function (t) {
                  i.disabled || c.clearActiveItem(),
                    c.helper.executeCallback.call(
                      c,
                      c.options.callback.onLeave,
                      [c.node, j(this), i, t]
                    );
                });
            })(0, n, r),
        ...
        'string' === typeof t
          ? (t = t.replace(
              ...
              function (t, e, i) {
                ...
                return (
                  ~(i = (i && i.split('|')) || []).indexOf('slugify') &&
                    (s = n.helper.slugify.call(n, s)),
                  s
                );
              }
            ))
    ...
    getMultiselectComparedData: function (t) {
      ...
      if (Array.isArray(this.options.multiselect.matchOn))
        for (var i = 0, s = this.options.multiselect.matchOn.length; i < s; ++i)
          e +=
            void 0 !== t[this.options.multiselect.matchOn[i]]
              ? t[this.options.multiselect.matchOn[i]]
              : '';
      else {
      ...
      if (this.options.hint)
        if (this.node[0].scrollWidth > Math.ceil(this.node.innerWidth()))
          ...
          if (
              ...
              for (var o, n, r, a = 0, l = t.length; a < l; a++)
                if (!t[a].disabled) {
                  ...
                  for (
                    var h = 0,
                      c = (o =
                        this.options.source[n].display || this.options.display)
                        .length;
                    h < c;
                    h++
                  )
                    if (
                      ((r = String(t[a][o[h]]).toLowerCase()),
                      this.options.accent &&
                        (r = this.helper.removeAccent.call(this, r)),
                      0 === r.indexOf(s))
                    ) {
            ...
            var p =
              (0 < i.length &&
                this.rawQuery + i.substring(this.query.length)) ||
              '';
            this.hint.container.val(p),
              this.isContentEditable && this.hint.container.text(p);
          }
        }
    },
                    ...
                    i.container.hasClass('filter') &&
                      j('html').on('click' + e + ' touchend' + e, function (t) {
                        (j(t.target).closest(
                          ...
                          j(t.target).closest(i.container)[0]) ||
                          i.hasDragged ||
                          (i.container.removeClass('filter'), j('html').off(e));
                      });
                },
              })
            ),
              j(this).append(
                j('<ul/>', { class: i.options.selector.dropdown })
              );
          },
    ...
    buildDropdownItemLayout: function (t) {
      if (this.options.dropdownFilter) {
        var e,
          ...
          o = this,
          n =
            ('string' === typeof this.options.dropdownFilter &&
              this.options.dropdownFilter) ||
            'All',
          r = this.container.find('.' + this.options.selector.dropdown);
        'static' !== t ||
          (!0 !== this.options.dropdownFilter &&
            'string' != typeof this.options.dropdownFilter) ||
        ...
        for (var s = 0, a = this.dropdownFilter[t].length; s < a; s++) {
          (i = this.dropdownFilter[t][s]),
            Array.isArray(i.value) || (i.value = [i.value]),
            i.all && (this.dropdownFilterAll = i.all);
            ...
            (l === h && s !== a - 1) ||
              ...
              i.value[l]
                ? (e = i.template
                    ...
                    : i.value[l])
                : this.container
                    .find('.' + o.options.selector.filterButton)
                    .html(e),
              (function (e, i, s) {
                r.append(
                  j('<li/>', {
                    class:
                      o.options.selector.dropdownItem +
                      ' ' +
                      o.helper.slugify.call(o, i.key + '-' + (i.value[e] || n)),
                    html: j('<a/>', {
                      href: 'javascript:;',
                      html: s,
                      click: function (t) {
      ...
      validate: function (t) {
        ...
        for (var n in this.filters.dynamic)
          if (
            ...
            ((i = ~n.indexOf('.')
              ? this.helper.namespace.call(this, n, t, 'get')
              : t[n]),
            '|' !== this.filters.dynamic[n].modifier ||
              s ||
              (s = i === this.filters.dynamic[n].value || !1),
            '&' === this.filters.dynamic[n].modifier)
          ) {
        ...
        return (
          (e = s), null !== o && !0 === (e = o) && null !== s && (e = s), !!e
        );
      },
      set: function (t, e) {
        var i = t.match(/^([|&])?(.+)/);
        e
          ? (this.filters.dynamic[i[2]] = { modifier: i[1] || '|', value: e })
          : delete this.filters.dynamic[i[2]],
          this.dynamicFilter.isEnabled && this.generateSource();
      },
      bind: function () {
        for (
          var t, e = this, i = 0, s = this.options.dynamicFilter.length;
          i < s;
          i++
        )
          'string' === typeof (t = this.options.dynamicFilter[i]).selector &&
            (t.selector = j(t.selector)),
              ...
              (function (t) {
                t.selector
                  ...
                  .on('change' + e.namespace, function () {
                    e.dynamicFilter.set.apply(e, [
                      t.key,
                      e.dynamicFilter.getValue(this),
                    ]);
                  })
        ...
        return (
          'SELECT' === t.tagName
            ...
            : 'INPUT' === t.tagName &&
              ('checkbox' === t.type
                ? (e =
                    (t.checked && t.getAttribute('value')) || t.checked || null)
    ...
    buildMultiselectLayout: function () {
      if (this.options.multiselect) {
        ...
        (this.label.container = j('<span/>', {
          ...
          click: function (t) {
            j(t.target).hasClass(e.options.selector.labelContainer) &&
              e.node.focus();
          },
        })),
          this.node
    ...
    addMultiselectItemLayout: function (t) {
      if (this.isMultiselectUniqueData(t)) {
        this.items.push(t),
          this.comparedItems.push(this.getMultiselectComparedData(t));
        var e,
            ...
            html: j('<' + o + '/>', {
              ...
              click: function (t) {
                var e = j(this).closest('.' + s.options.selector.label),
                  i = s.label.container
                    .find('.' + s.options.selector.label)
                    .index(e);
                s.options.multiselect.callback &&
                  s.helper.executeCallback.call(
                    s,
                    s.options.multiselect.callback.onClick,
                    [s.node, s.items[i], t]
                  );
              },
              href: this.options.multiselect.href
                ? ((e = s.items[s.items.length - 1]),
                  s.generateHref.call(s, s.options.multiselect.href, e))
                : null,
            }),
          });
        return (
    ...
    adjustInputSize: function () {
      ...
      this.label.container
        ...
        .filter(function (t, e) {
          0 === t &&
            (a =
              j(e)[0].getBoundingClientRect().height +
              parseFloat(j(e).css('margin-bottom') || 0)),
            (s =
              j(e)[0].getBoundingClientRect().width +
              parseFloat(j(e).css('margin-right') || 0)),
            0.7 * i < n + s && !r && (o++, (r = !0)),
            n + s < i ? (n += s) : ((r = !1), (n = s));
        });
      var t =
          ...
          j('html')
            ...
            .on('keydown' + this.namespace, function (t) {
              t.keyCode &&
                9 === t.keyCode &&
                setTimeout(function () {
                  j(':focus').closest(e.container).find(e.node)[0] ||
                    e.hideLayout();
                }, 0);
            }),
            j('html')
              ...
              .on(
                ...
                function (t) {
                  j(t.target).closest(e.container)[0] ||
                    j(t.target).closest('.' + e.options.selector.item)[0] ||
                    t.target.className === e.options.selector.cancelButton ||
                    e.hasDragged ||
                    e.hideLayout();
                }
              );
    ...
    hideLayout: function () {
      (this.container.hasClass('result') ||
        ...
        (this.container.removeClass(
          'result hint filter' +
            (this.options.backdropOnFocus && j(this.node).is(':focus')
              ? ''
              : ' backdrop')
        ),
        (this.options.backdropOnFocus && this.container.hasClass('backdrop')) ||
          (j('html').off(this.namespace),
          this.helper.executeCallback.call(
            this,
        ...
        j('<span/>', {
          ...
          mousedown: function (t) {
            t.stopImmediatePropagation(),
              t.preventDefault(),
              e.resetInput(),
              e.node.trigger('input' + e.namespace, [t]);
          },
    ...
    helper: {
      ...
      removeAccent: function (t) {
        if ('string' === typeof t) {
          ...
          return (
            'object' === typeof this.options.accent &&
              (e = this.options.accent),
            (t = t
              .toLowerCase()
              .replace(new RegExp('[' + e.from + ']', 'g'), function (t) {
      ...
      highlight: function (t, e, i) {
        ...
        for (n = o.length - 1; 0 <= n; n--)
          t = this.helper.replaceAt(
            t,
            o[n].offset,
            o[n].length,
            '<strong>' + t.substr(o[n].offset, o[n].length) + '</strong>'
          );
        return t;
      },
      getCaret: function (t) {
        ...
        if (document.selection) {
          ...
          var s = t.createTextRange(),
            o = s.duplicate();
          s.moveToBookmark(i.getBookmark()),
      ...
      executeCallback: function (t, e) {
        if (t) {
          ...
          if ('function' === typeof t) i = t;
            ...
            ('string' === typeof t && (t = [t, []]),
            'function' !=
              typeof (i = this.helper.namespace.call(this, t[0], window)))
          )
            return;
      ...
      if (e && e.source && 'object' === typeof e.source) {
        ...
        if ((void 0 === t[0].value && (t[0].value = t.text()), t.length)) {
          if (1 === t.length)
            return (
              (t[0].selector =
                t.selector || e.input || t[0].nodeName.toLowerCase()),
              (window.Typeahead[t[0].selector] = new l(t, e))
            );
          for (var i, s = {}, o = 0, n = t.length; o < n; ++o)
            void 0 !== s[(i = t[o].nodeName.toLowerCase())] && (i += o),
              (t[o].selector = i),
              (window.Typeahead[i] = s[i] = new l(t.eq(o), e));
          return s;
        }
      }
    },
